...
scrape_with_session <- function(rds_filename = "baseball_transactions_session.rds") {
  ...
  cookies <- c(
    "cf_clearance" = "Tp7kr.gmq09OO.V4KXqPlXKZ5YHRXFL7eeUHN9ZQ5co-1751668734-1.2.1.1-8eZqlEsBXy5nUIHzdZGy89m89BW.bnITnniUN50l3vw0LPNhA6iTHhaZKY.CGTwd8Gp15UtYtebmqRHdZ76gFBS5bNH2uKp.GPPojAd787YUfca4O3ps33Uv2d4Em4Bf7cUO0Cgeo0ksNsSpQUkSjlx.YFEOQxvwJBfoznsj_WlcO4Jlpn8cMkfemEBEPLE_jJsosfAojlKz7owNCXQgmgSU1KI6lEsFP_4QwSrvwNQx3sZxO4JjzLKdcR1KOR2i"
  )
  ...
  # Test the session
  test_url <- "https://www.prosportstransactions.com/baseball/Search/SearchResults.php?Player=&Team=&BeginDate=2025-03-14&EndDate=2025-07-02&PlayerMovementChkBx=yes&submit=Search"
    ...
    if (status_code(response) == 200) {
      ...
      if (length(tables) > 0) {
        ...
        if (nrow(each_page_df) > 1) {
          ...
          saveRDS(all_injuries, "baseball_transactions_session.rds")
          cat("Data saved to baseball_transactions_session.rds (", nrow(all_injuries), "total rows)\n")
        }
      }
    } else {
  ...
  # Save results
  saveRDS(all_injuries, "baseball_transactions_session.rds")
  cat("Data saved to baseball_transactions_session.rds\n")
...
result <- readRDS("baseball_transactions_session.rds")
...
result <- result %>%
  filter(
    !grepl("released \\(from (R|A|AA|AAA|Adv A)\\)", Notes),
    !grepl("promoted", Notes),
    !grepl("hired", Notes),
    !grepl("fired", Notes),
    !is.na(Team),
    !grepl("waived \\(from (R|A|AA|AAA|Adv A)\\)", Notes),
    !grepl("retired \\(from (R|A|AA|AAA|Adv A)\\)", Notes)
  ) |>
  unique()

result$Date <- as.Date(result$Date)

saveRDS(result, "baseball_transactions.rds")
